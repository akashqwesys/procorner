<div class="col-md-12">
    <div class="load-bundle-card pb-3">
            ...
            <?php foreach($courses as $key => $course_details): ?>
                <div class="accordion mb-2" id="<?= 'example_'.$bundle_details['id'].$course_details['id']; ?>">
                    <a href="<?php echo site_url('home/course/'.rawurlencode(slugify($course_details['title'])).'/'.$course_details['id']); ?>" target="_blank">
                        <div class="card">
                            <div class="card-header collapsed p-0" type="button" data-toggle="collapse" data-target="#<?= 'course_'.$bundle_details['id'].$course_details['id']; ?>" aria-expanded="false" aria-controls="<?= 'course_'.$bundle_details['id'].$course_details['id']; ?>">
        ...
        <div class="row">
            <div class="col-md-12">
                <a href="<?= site_url('bundle_details/'.$bundle_details['id'].'/'.slugify($bundle_details['title'])); ?>" class="btn-bundle-details btn btn-primary"><?= site_phrase('bundle_details'); ?></a>

                <?php if(get_bundle_validity($bundle_details['id'], $this->session->userdata('user_id')) == 'invalid'): ?>
                    <a href="<?= site_url('course_bundles/buy/'.$bundle_details['id']); ?>" class="btn-buy-bundle btn btn-primary"><?= site_phrase('buy'); ?></a>
