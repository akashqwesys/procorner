		...
		<div class="row justify-content-center">		
				...
				<div class="row">
					...
					<div class="col p-0">
						<div class="course-box-wrap">
							<a href="<?php echo site_url(rawurlencode(slugify($course['title']))); ?>">
			...
			<div class="col-lg-9">
				...
				<?php if(get_bundle_validity($bundle_details['id'], $this->session->userdata('user_id')) == 'invalid'): ?>
					<a href="<?= site_url('course_bundles/buy/'.$bundle_details['id']); ?>" class="btn btn-primary ms-auto col-2" style="float:right"><?= currency($bundle_details['price']); ?> | <?= site_phrase('buy'); ?></a>
				<?php elseif(get_bundle_validity($bundle_details['id'], $this->session->userdata('user_id')) == 'expire'): ?>
					<a href="<?= site_url('course_bundles/buy/'.$bundle_details['id']); ?>" class="btn btn-primary ms-auto"><?= currency($bundle_details['price']); ?> | <?= site_phrase('renew'); ?></a>
				<?php else: ?>
					<a href="<?= site_url('home/my_bundles'); ?>" class="btn btn-primary ms-auto"><?= site_phrase('purchased'); ?></a>
				<?php endif; ?>
		</div>
		</div>		
	</div>
